...
class GPTModel(LanguageModule):
    ...
    def __init__(
        ...
        if self.post_process:
            ...
            if self.config.defer_embedding_wgrad_compute:  #开启 defer_embedding_wgrad_compute 后，反传时不立即算 wgrad（LM Head的embedding参数梯度，激活值的梯度当然会算），而是把 embedding 输入激活值暂存到 embedding_activation_buffer（tensor_parallel/layers.py:1034 附近），等step内所有 micro-batch 攒齐后再一次性批量算（drain_embedding_wgrad_compute）。
