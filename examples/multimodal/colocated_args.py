...
def default_round_robin_partition(num_microbatches: int, num_producers: int) -> list[list[int]]:
    """Default colocated-encoder microbatch partition: round-robin over producers.

    默认划分：producer p 负责 p, p+P, p+2P, ...；第 i 项为 producer i（producer 0 即 consumer）
    的 mb id 升序表。即使 num_microbatches 不整除 num_producers 也是精确划分（低编号多分一个）。
    签名 ``(num_microbatches, num_producers) -> list[list[int]]``。
    """
    return [
        list(range(producer_id, num_microbatches, num_producers))
